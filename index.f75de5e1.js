...
removeRow.addEventListener('click', ()=>{
    if (tbody.children.length > minRows) tbody.lastElementChild.remove();
    updateButton();
});
appendColumn.addEventListener('click', ()=>{
    const rows = table.rows;
    if (rows[0].cells.length < maxCols) for(let i = 0; i < rows[0].cells.length; i++)rows[i].insertCell();
});
removeColumn.addEventListener('click', ()=>{
    const rows = table.rows;
    if (rows[0].cells.length > minCols) for(let i = 0; i < rows.length; i++)rows[i].deleteCell(-1);
    updateButton();
});
